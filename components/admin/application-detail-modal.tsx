...
export function ApplicationDetailModal({ application, open, loading = false, onClose }: ApplicationDetailModalProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      {/* 모달 사이즈 대폭 확장: max-w-[90vw] 및 최대 1600px 제한 */}
      <DialogContent className="max-w-[70vw] xl:max-w-[1400px] max-h-[92vh] overflow-hidden flex flex-col bg-black/95 border border-white/20 text-white p-0 shadow-2xl backdrop-blur-2xl">
            ...
            <>
              ...
              <div className="bg-white/5 border border-white/10 rounded-[32px] p-8 hover:bg-white/[0.07] transition-colors">
                ...
                {app.electronicDevices && app.electronicDevices.length > 0 && (
                  <div className="mt-10 pt-8 border-t border-white/10 space-y-4">
                    <p className="text-sm font-black text-amber-500/80 uppercase tracking-widest">PERSONAL DEVICES</p>
                ...
                <div className="bg-white/5 border border-white/10 rounded-[32px] p-8 transition-colors">
                  ...
                  <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                    {app.companions.map((companion: any, idx: number) => (
                      <div key={idx} className="bg-black/40 border border-white/10 rounded-3xl p-6 space-y-6">
                        ...
                        {companion.electronicDevices && companion.electronicDevices.length > 0 && (
                          <div className="pt-4 border-t border-white/10">
                            <p className="text-[10px] font-bold text-white/40 mb-3 uppercase">Device List</p>
              ...
              {app.files && app.files.length > 0 && (
                <div className="bg-white/5 border border-white/10 rounded-[32px] p-8">
                  <h3 className="text-xl font-black text-white mb-8 flex items-center gap-3">
                    <span className="p-2 bg-green-500/20 rounded-xl text-green-400 text-sm">04</span>
                    첨부파일 ({app.files.length})
                  </h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {app.files.map((file: any, idx: number) => (
                      <div key={idx} className="flex items-center justify-between bg-black/40 border border-white/10 rounded-2xl p-4 group hover:border-amber-500/50 transition-all">
                        <div className="flex items-center gap-4 min-w-0">
                          <div className="w-12 h-12 bg-white/5 rounded-xl flex items-center justify-center text-2xl group-hover:scale-110 transition-transform">📄</div>
                          <div className="min-w-0">
                            <p className="text-sm font-bold text-white truncate">{file.filename}</p>
                            <p className="text-xs text-white/30">FILE ATTACHMENT</p>
                          </div>
                        </div>
                        {(file.key || file.url) && (
                          <Button
                            onClick={() => window.open(`/api/files/${encodeURIComponent(file.key || file.url)}`, '_blank')}
                            size="icon"
                            className="bg-amber-500/10 hover:bg-amber-500 text-amber-500 hover:text-black rounded-xl transition-all"
                          >
                            <Download size={20} />
                          </Button>
                        )}
              ...
              {app.rejection_reason && (
                <div className="bg-red-500/10 border-2 border-red-500/30 rounded-[32px] p-8">
                  <h3 className="text-xl font-black text-red-400 mb-4 flex items-center gap-3">
