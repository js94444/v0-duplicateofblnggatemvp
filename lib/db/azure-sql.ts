...
export class AzureSqlDB {
  ...
  static async createPassForApplication(applicationId: string, pass_receipt: string): Promise<void> {
    ...
    const validFrom = app?.visit_start_date || new Date()
    const validUntil = app?.visit_end_date || new Date()
    ...
    await dbPool.request()
      ...
      .input('valid_until', sql.DateTime, validUntil)
      .query(`
        INSERT INTO visit_passes (application_id, pass_receipt, token, status, valid_from, valid_until)
        VALUES (@application_id, @pass_receipt, @token, 'active', @valid_from, @valid_until)
      `)
  }
